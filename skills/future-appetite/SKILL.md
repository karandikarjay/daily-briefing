...
Discover broadly before choosing the lineup. Search the major coverage areas separately and inspect relevant specialist news pages, primary announcements, research releases, and FAST; one broad search or a few familiar sources is not enough. Follow up on promising omissions before settling on a short edition. Record credible contenders, including those left out for space or editorial priority, with a brief reason in `sources.md`. Distinguish an eligible story judged less important from an old, duplicate, or unverified item. A new substantive investigation, analysis, or implementation milestone can qualify even when its underlying topic has appeared before.

When Jay requests a prioritization review, reconstruct that edition's exact window and present a broad, deduplicated candidate list with concise summaries, sources, timing, and whether each was included. Keep unresolved timing and out-of-window stories separate. Ask which stories he would lead with, include briefly, or omit, and why; translate his answers into durable editorial preferences without inventing topic quotas or treating one day's ranking as a permanent rule. This review is interactive when requested, not a prerequisite for scheduled delivery.
...
Qualify a claim when the qualification changes its meaning or the reader's assessment. Avoid habitual disclaimers and generic reminders that an announcement does not prove impact. Accurate, attributed wording often supplies the needed distinction without an extra cautionary sentence. Keep routine methodology and verification details in private notes.
...
Generate conceptual photorealistic illustrations for substantial stories; brief updates may omit them. Preferred image model: `gpt-image-2.5-sunburst`, medium quality, landscape 1536×1024. Captions simply describe what the image depicts. Do not append “Conceptual illustration,” “AI-generated,” “artist's impression,” or similar labels. Choose imagery and caption wording that do not imply a photograph of the actual event, people, or facility. Use a native image tool when available, otherwise the image API with the existing credential. Do not put FAST/private material into image prompts. Inspect generated images before inclusion. A failed illustration must not prevent a useful email; omit it and note the failure privately.

Attempt the three regular charts: Beyond Meat stock (BYND), Oatly stock (OTLY), and S&P 500. Prefer roughly a year of observations, adjusted daily close for equities when available. Verify adjustments and record them in private source notes; show observation dates clearly. Do not routinely explain adjusted closes, dividend exclusion, or historical reverse splits in reader copy. Mention a methodological detail only when needed to understand an unusual move or avoid a materially misleading comparison. Useful starting sources are Yahoo Finance chart data for BYND, OTLY and `^GSPC`; you may find better accessible sources.

Keep observation dates distinct from retrieval dates; exclude information released after the reporting cutoff. Render genuine chart data with a plotting tool or SVG converted to PNG with `rsvg-convert`; AI-generated images must never stand in for measured charts. Save observations and source URLs locally. Add a short useful observation for each included chart within the word budget. Do not invent causes for price moves or equate them with consumer demand; enforce this through sound analysis rather than a recurring disclaimer that stocks do not measure demand. Omit and privately record unavailable, blank, or misleading charts. Never reuse stale image files from earlier runs. Embed included PNG/JPEG images as MIME Content-ID attachments, with a local-path HTML version for preview inspection if useful.
...
During the final copy edit, remove illustration labels and routine chart-methodology or generic cautionary sentences. Retain qualifications that materially change the interpretation of a specific claim. Compare the chosen lineup with the strongest omitted contenders before finalizing it.
